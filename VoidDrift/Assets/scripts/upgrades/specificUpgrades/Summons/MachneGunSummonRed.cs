    ...
    public override void function()
    {
        if(level == 0)
        {
           if(SummonsManager.instance.addSummon(summon,out pom))
           {
                ...
                description = string.Format("Drone cooldown - 20% Drone base damage + {0}",Damage1);
            }
        }else
        if(level == 1)
        {
            ...
            description = string.Format("Drone projectiles + 3 projectile speed + 40%");
            
        }else if(level == 2)
        {
            drone.bulletsInBurst += 3;
            drone.BaseForce = drone.BaseForce + (drone.BaseForce * 0.4f);
            description = string.Format("Drone cooldown - 30% base damage + {0}",Damage2);
